...
// 对点云数据进行时间压缩。将点云中的点按照相邻点的时间戳进行分组，从而能够更高效地处理数据。
// 这里使用点的curvature属性来表示时间戳
template <typename T>
std::vector<int> time_compressing(const PointCloudXYZI::Ptr &point_cloud) {
    // time_seq向量将用于存储每组点的数量?好像确实,
    std::vector<int> time_seq;
    // time_seq.clear();
    int points_size = point_cloud->points.size();
    int j = 0;
    time_seq.reserve(points_size);  // 最多一一分组
    // 分组,计数
    for (int i = 0; i < points_size - 1; i++) {
        j++;
        // 检查下一个点的时间值是否大于当前点。如果是(该组分组完成)，则将j的值添加到time_seq向量中，并将j重置为0
        if (point_cloud->points[i + 1].curvature > point_cloud->points[i].curvature) {
            time_seq.emplace_back(j);
            j = 0;
        }
    }
    // 检查j的值。如果为0，则在time_seq向量中添加1；否则，将j+1添加到time_seq向量中。 估计是修正j++
    if (j == 0)
