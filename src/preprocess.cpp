...
void Preprocess::process(const livox_ros_driver2::msg::CustomMsg::UniquePtr &msg, CloudType::Ptr &pcl_out) {
    avia_handler(msg);
    *pcl_out = pl_surf;
}

void Preprocess::process(const sensor_msgs::msg::PointCloud2::UniquePtr &msg, CloudType::Ptr &pcl_out) {
    switch (time_unit) {
        case SEC:
...
void Preprocess::oust64_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg) {
    ...
    double time_stamp = msg->header.stamp.sec;  // nanosec * 1e-9;  // get_time_sec(msg->header.stamp); //msg->header.stamp.toSec();
    // cout << "===================================" << endl;
...
int Preprocess::plane_judge(const CloudType &pl, vector<orgtype> &types, uint i_cur, uint &i_nex, Eigen::Vector3d &curr_direct) {
...
bool Preprocess::edge_jump_judge(const CloudType &pl, vector<orgtype> &types, uint i, Surround nor_dir) {
    if (nor_dir == 0) {
